...
def process_database(db_path):
    ...
    "all":{"runs":10, "start_run_number":1}
}
    if db_path == 'standard':
        ...
        db_info = db_path_to_variable.get('all', None)
    ...
    for run in range(1,number_of_runs+1):
        ...
        df['gene_expression'] = df['genotype'].apply(lambda x: x.develop(include_bias = config.CPPNBIAS,
...
def main() -> None:
    ...
    elif config.DATABASE_FILE == 'nocross':
...
# Group by 'generation' and compute the average length
    ...
    avg_gene_expression=('gene_expression', 'mean')
    ).reset_index()
    print(result)
