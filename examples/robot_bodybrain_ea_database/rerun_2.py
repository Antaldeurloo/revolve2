...
def main() -> None:
    ...
    dbengine = open_database_sqlite(
        'testfile.sqlite', open_method=OpenMethod.OPEN_IF_EXISTS
    )
    ...
    with Session(dbengine) as ses:
        rows = ses.execute(
            ...
            .where(Generation.generation_index < 164)
...
# Primary axis for body length
    ...
    ax1.set_ylabel('Average Body Length', fontsize=12, color='blue')
    ax1.plot(result['generation_index'], result['avg_body_length'], marker='o', linestyle='-', linewidth=2, color='blue', label='Average Body Length')
    ax1.tick_params(axis='y', labelcolor='blue')
    ax1.grid(True, linestyle='--', alpha=0.6)

# Secondary axis for fitness
    ax2 = ax1.twinx()
    ax2.set_ylabel('Average Fitness', fontsize=12, color='green')
    ax2.plot(result['generation_index'], result['avg_fitness'], marker='s', linestyle='--', linewidth=2, color='green', label='Average Fitness')
    ax2.tick_params(axis='y', labelcolor='green')

# Add title and legends
    fig.suptitle('Average Body Length and Fitness by Generation Index', fontsize=14)
